...
|---|---|---|---|
| v1.19.0 | Registry Bulk Operations Agent | NL bulk updates across multiple registry items — "close all plumbing items", "mark all stale as in-progress", "set all HU/HI electrical items to in-progress"; extends current single-item update agent with set-based operations | Batch ticket processing, bulk status normalization |
| v1.19.0 | Memory / Context Agent | Orchestrator references past run decisions from `run_history` — "this item has been HU/HI for 3 consecutive runs", "last time we saw this pattern we deferred it"; episodic memory layered on existing `run_history` table | Institutional memory for recurring risk patterns, time-to-resolution tracking |
...
| OpenAI | o3 / o3-mini | Structured reasoning over schemas, math-heavy scoring _(future)_ | Chain-of-thought reasoning with explicit steps; natural fit for schema metric discovery and scoring rubric agents |
